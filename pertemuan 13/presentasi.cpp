...
const int INF = 1e9; // Nilai tak hingga untuk penanda tidak ada jalur
...
// Pemetaan nama ke indeks
map<string,int> idx;

// Matriks untuk menyimpan jalur Floyd-Warshall
...
// Algoritma Dijkstra: mengembalikan jarak dan parent
...
// Menambahkan edge tak berarah dengan bobot w
...
int main() {
    ...
    if (fitur == 1) {
        ...
        auto sol = dijkstra(adj, src);
        vector<int> dist = sol.first, parent = sol.second;

        if (dist[dst] >= INF) {
            cout << "Tidak ada jalur dari " << ssrc << " ke " << sdst << ".\n";
        } else {
            vector<int> path;
